...
from core.skills import get_skill, build_few_shot_block
...
async def extractor_node(state: SimPipelineState, tools=None):
    """参数提取 + 校验一体化节点（内置 Few-Shot Prompt 注入，合并了原 ParamValidator）"""
    try:
        ...
        # ✨ Few-Shot 注入：从技能定义中读取示例，拼接到 Prompt 末尾
        few_shot_block = build_few_shot_block(skill_info.get("few_shot_examples", []))
        if few_shot_block:
            final_system_prompt += few_shot_block
        ...
        print(f"[Extractor] 📤 确认数据提取完成，流转至 Critic 智能体进行校验...")
