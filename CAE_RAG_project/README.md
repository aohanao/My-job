...
### `file_history_store.py` — 带有摘要压缩的会话存储

这是系统的“短期记忆调度中心”，负责管理用户与 CAE 专家的多轮对话历史，并确保在长对话下不爆掉大模型的 Token 上限。
- **滑窗摘要机制 (Sliding Window Summary)**：
    - **触发阈值**：当 `messages` 数量超过 `max_messages` (默认 6 条，约 3 轮对话) 时，系统会自动启动“冷酷的记忆洗牌”。
    - **切片分流**：系统执行“留四去多”策略，保留最近的 2 轮对话作为精确上下文，将其余历史消息投入“摘要高压锅”。
- **折叠式总结逻辑**：
    - **信息提纯**：调用大模型对旧对话进行“折叠式总结”，提取其中达成共识的参数条件、工程场景特征以及最后决定的规范条目。
    - **滚雪球式演进**：新的摘要会将之前的“前文脉络”一并吞入重新炼丹，确保记忆链条不断联。
- **系统提示词注入**：
    - 总结后的“高密结晶”会以 `SystemMessage` 的形式重新注入历史记录的首位。
    - 带有明显的提示语：“【前文情境摘要脉络】：...请在回答最新提问时，不要遗忘上面的大背景事实。”
- **文件持久化**：
    - 采用 JSON 序列化方案，将对话按 `session_id` 存储在本地 `/chat_history` 目录下，实现会话的跨重启续接。
    - 暴露了 `clear_history_by_id` 接口，支持侧边栏一键清空记忆。

### `semantic_cache.py` — 长记忆缓存盾牌与经验大坝

这是 RAG 系统的核心降本组件，负责管理系统的“长期经验记忆”。
- **语义缓存机制**：不同于关键词匹配，它利用向量相似度截获“意思相近”的提问。若历史经验中存在高度重合的答案，则直接返回。
- **长期记忆管理**：
    - **存储实现**：采用独立的微型 Chroma 集合持久化存储（Question, Answer）对。
    - **资产化**：将高质量的大模型回答转化为系统的永久资产，不随会话结束而消失。
- **性能优势**：实现 0 Token 消耗响应，将复杂的 RAG 链路缩短至毫秒级。
