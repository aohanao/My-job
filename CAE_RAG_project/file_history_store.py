...
class FileLock:
    """纯 Python 实现的跨平台轻量级文件排它锁，含死锁恢复机制"""
    def __init__(self, file_path, timeout=5.0, delay=0.05):
        ...
        while True:
            try:
                ...
                if time.time() - start_time > self.timeout:
                    # 发生超时，检查锁文件是否为死锁残留（若存在时间超过 10 秒则强制清理）
                    try:
                        ...
                        pass
                    raise TimeoutError(f"[FileLock] 获取文件锁超时，锁定资源: {self.lock_file}")
...
class SummaryFileChatMessageHistory(BaseChatMessageHistory):
    ...
    def add_message(self, message: BaseMessage) -> None:
        with FileLock(self.file_path):
            ...
            try:
                with open(self.file_path, "r", encoding="utf-8") as f:
                    all_messages = messages_from_dict(json.load(f))
            except FileNotFoundError:
                pass
